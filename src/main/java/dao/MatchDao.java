...
public class MatchDao implements Dao<Long, Match> {
    ...
    public static final String FROM_MATCH_HQL = "from Match";
    public static final String FROM_MATCH_PAGINATION_HQL = "from Match " +
                                                           "where player1.name ilike :filterByPlayerName " +
                                                           "or player2.name ilike :filterByPlayerName";
    ...
    public List<Match> findAllPaginated(int offset, int maxResults) {
        try (Session session = HibernateUtil.getSessionFactory().getCurrentSession()) {
            session.beginTransaction();
            List<Match> finishedMatches = session.createQuery(FROM_MATCH_HQL, Match.class).setFirstResult(offset).setMaxResults(maxResults).list();
            session.getTransaction().commit();
    ...
    public List<Match> findFilteredByPlayerNamePaginated(int offset, int maxResults, String filterByPlayerName) {
        try (Session session = HibernateUtil.getSessionFactory().getCurrentSession()) {
            session.beginTransaction();
            List<Match> filteredFinishedMatches = session.createQuery(FROM_MATCH_PAGINATION_HQL, Match.class)
